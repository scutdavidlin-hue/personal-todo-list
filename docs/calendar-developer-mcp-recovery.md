...
Local implementation is ready for a narrow deployment of the existing `google-tasks` and `task-scheduler` functions only. No migration or unrelated function is required. A read-only management API check on 2026-09-08 confirmed that production remains on `google-tasks` v12 and `task-scheduler` v10; both existing functions have `verify_jwt=false`. Automatic approval review rejected a production deploy that preserved that setting because explicit authorization for the production deployment and security setting was considered missing. The fix is therefore committed to GitHub but not deployed. Production acceptance must be read-only or use a dedicated synthetic fixture. Real travel events and the two observed date-conflict Tasks must not be modified.
