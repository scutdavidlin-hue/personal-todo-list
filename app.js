  ...
  return `<article class="goal-card ${financial ? "financial" : ""}">
    <button type="button" class="goal-card-open" data-open-goal="${escapeHtml(goal.id)}" aria-label="打开 ${escapeHtml(goal.title)}">
      <div class="goal-card-heading">
        <span class="goal-kind">${escapeHtml(GOAL_TYPE_LABELS[goal.type] || goal.type)} · ${escapeHtml(GOAL_CATEGORY_LABELS[goal.category] || goal.category)}</span>
  ...
  $("#recentGoals").innerHTML = visible.length
    ? visible.map((goal) => {
      ...
      return `<button type="button" class="recent-goal" data-open-goal="${escapeHtml(goal.id)}">
        <span>${escapeHtml(GOAL_CATEGORY_LABELS[goal.category] || goal.category)}</span>
...
function renderGoalDetail() {
  ...
  $("#goalDetailKind").textContent = `${GOAL_TYPE_LABELS[goal.type] || goal.type} · ${GOAL_STATUS_LABELS[goal.status] || goal.status}`;
