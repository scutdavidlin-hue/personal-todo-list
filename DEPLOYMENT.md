...
代码已完成，但真实 Google 账号测试必须先在用户自己的 Google Cloud 与 Supabase 项目完成以下一次性配置。仓库不包含任何真实凭证。

## 1. Google Cloud

1. 在现有 Google Cloud 项目启用 **Google Tasks API**。
2. 在 Google Auth Platform 的 Data Access 中加入：
   - `openid`
   - `.../auth/userinfo.email`
   - `.../auth/userinfo.profile`
   - `https://www.googleapis.com/auth/tasks`
   - 保留现有 Calendar scope，不要删除或替换。
...
三个函数都在代码中自行验证调用方：`google-tasks` 验证 Supabase 用户 JWT；`task-status` 与 `action-router` 使用独立的恒定时间 Token 比较。
...
window.TASK_SYNC_CONFIG = Object.freeze({
  ...
  googleOAuthScopes: "https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/tasks",
});
```
...
5. 在 Google Calendar 确认 **Tasks 日历已勾选显示**，然后确认同一天能看到该任务。Google Calendar 是否展示 Tasks 是用户界面设置，不是 Tasks API 的第二次写入。
6. 回到页面勾选完成；再次刷新手机 Google Tasks 与 Calendar，确认状态同步为已完成。
