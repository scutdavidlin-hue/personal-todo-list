...
const GOAL_PATTERNS = /(?:(?:20\d{2}\s*年|明年|后年|未来|长期|以后).{0,28}(?:想|希望|目标|完成|实现|升级|达到|成为|商业化)|(?:目标是|目标：|希望在).{1,60})/;
const PLAN_PATTERNS = /(?:(?:计划|打算).{1,80}|(?:20\d{2}\s*年|明年|后年|\d{1,2}\s*[–—~-]\s*\d{1,2}\s*月|\d{1,2}\s*月).{0,20}(?:开始|推进|筹备|规划).{1,60})/;
...
function longTermPayload(text, itemType, baseDate) {
  ...
  return {
    ...
    targetYear,
    targetMonth: parseTargetMonth(text, targetYear, baseDate),
