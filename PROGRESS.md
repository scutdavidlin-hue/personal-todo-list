...
- [ ] 生产窄部署与安全验收待完成。只允许发布既有 `google-tasks`、`task-scheduler`；不得部署 migration/无关 Function，不得修改真实旅行事件或两条真实 date-conflict Task 日期。2026-09-08 只读回读确认线上仍为 `google-tasks` v12、`task-scheduler` v10，二者既有配置均为 `verify_jwt=false`；自动审批审查拒绝了沿用该配置的生产部署，理由是缺少对生产部署及安全设置的显式授权，因此本地验证与 GitHub 提交不等于生产已发布。
